""""
该方法为ipcc方法预测用户评分，且将无法预测的分数使用平均分填充
"""
...
class ipcc:
    ...
    def calculate_sigmoid_coefficient(self):
        ...
        # 每两个用户共同打分的项目个数矩阵
        both_rating_num = np.zeros((self.item_count, self.item_count))
    ...
    def calculate_pearson_similarity_matrix(self):
        ...
        :return:
        """
        item_average_data = np.average(self.data, axis=1).reshape(self.item_count, 1)
        variance = self.data - item_average_data

        covariance_matrix = np.dot(variance, variance.T)

        square_variance = np.sum(np.square(variance), axis=1).reshape(self.item_count, 1)
    ...
    def findSimilarItems(self, index, top=5):
    ...
    def evaluate(self, indices):
        ...
        num_of_predicted = 0
        num_of_failure = 0
        # 使用predicted_matrix记录经过ipcc方法预测后的矩阵，且无法预测的值暂时不用矩阵填充
        predicted_matrix = np.copy(self.data)
        # 初始化所有商品均未被覆盖
        coverage_flag = [False for x in range(self.data.shape[0])]
        # 求所有商品的平均分数
        valid_flag = self.data > 0
        valid_flag_count = np.sum(valid_flag, axis=1).reshape(self.item_count, 1)
        valid_flag_count[valid_flag_count == 0] = 1
        items_avg_value = np.sum(self.data, axis=1).reshape(self.item_count, 1) / valid_flag_count
        # 若用户没有任何打分，则将平均数置为全局平均数
        items_avg_value[items_avg_value == 0] = np.sum(self.data) / np.sum(self.data > 0)
        ...
        for idx in indices:
            ...
            for (i, flag) in enumerate(predicted_columns):
                if flag == True:
                    predicted_users.append(i)
            predicted_users = np.array(predicted_users)
            similar_items, similarities = self.findSimilarItems(idx, self.top)
            #           提取相似用户的评分
            data_for_predicted = self.data[similar_items, :]
            ...
            if np.sum(valid_counts) == 0:
                ...
                predicted_values[:, :] = items_avg_value[idx]
                ...
                # 将可以预测出的商品置为True
                coverage_flag[idx] = True
                for (i, value) in enumerate(predicted_values[0]):
                    if value > 0: predicted_matrix[idx][predicted_users[i]] = value
                predicted_values[predicted_values == 0] = items_avg_value[idx]

            predicted_values = predicted_values.squeeze()
            # print('对第%d商品的预测分数:%s' % (idx, predicted_values))
        ...
        return mae / num_of_predicted, np.sqrt(rmse / num_of_predicted), num_of_failure / num_of_predicted, np.sum(
            np.array(coverage_flag)) / self.data.shape[0],  predicted_matrix.T
...
if __name__ == '__main__':
    with open('../../dataset/preprocessed_data_for_0.2ratio.json', 'r') as f:
        ...
        np.random.seed(1)
        indices = np.random.choice(range(data.shape[1]), 300, replace=False)
        ipcc_algorithm = ipcc(raw_data, data, top=5)
        matrix = ipcc_algorithm.calculate_pearson_similarity_matrix()
        mae, rmse, failureRate,coverage,predicted_matrix =ipcc_algorithm.evaluate(indices)
        print('mae= ',mae,'rmse= ',rmse,'failureRate= ',failureRate,'coverage= ',coverage)
        print('predicted matrix = ')
        print(predicted_matrix)
        # with open('../../output/datafile_itself/K_choice/more_ipcc_predicted_matrix_top=15.json', 'w')as fp:
        ...
        # print(mae, rmse, failureRate)
        # #存储皮尔逊相似度矩阵
        # with open('../../dataset/similarity/ipcc_similarity_matrix_0.2ratio_withCoefficient.json', 'w') as fp:
